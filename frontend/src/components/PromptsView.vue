...
<template>
  <section class="prompts-page" data-theme="snowfall">
    ...
    <div v-else class="prompts-list"><article v-for="entry in visibleEntries" :key="entry.id" class="prompt-entry prompt-snow-surface"><div class="prompt-entry-heading"><div><span class="prompt-entry-category">{{ entry.category || "未分类" }}</span><h2>{{ entry.name }}</h2></div><time>{{ new Date(entry.updated_at).toLocaleDateString('zh-CN') }}</time></div><p>{{ entry.prompt }}</p><div class="prompt-entry-actions"><button type="button" class="primary-action" @click="applyEntry(entry)"><Sparkles :size="15" />应用到工作台</button><button type="button" class="secondary-action" @click="openEdit(entry)"><Edit3 :size="15" />编辑</button><button type="button" class="icon-action danger-action" aria-label="删除提示词" title="删除提示词" @click="deleting = entry"><Trash2 :size="16" /></button></div></article></div>
...
<style scoped>
.prompts-page { max-width:1100px; min-height:calc(100dvh - 64px); margin:0 auto; padding:32px 48px 56px; color:var(--prompt-snow-text); background:var(--prompt-snow-page); }.prompts-heading { display:flex; justify-content:space-between; align-items:flex-end; gap:24px; padding-bottom:22px; border-bottom:1px solid var(--prompt-snow-border); }.prompts-kicker { display:flex; gap:7px; align-items:center; color:var(--prompt-snow-accent); font-size:11px; letter-spacing:.08em; }.prompts-heading h1 { margin:7px 0 5px; font-size:30px; line-height:1.15; }.prompts-heading p { margin:0; color:var(--prompt-snow-text-muted); font-size:13px; }.prompts-heading-actions,.prompts-toolbar,.prompt-entry-actions { display:flex; align-items:center; gap:8px; }.prompts-heading-actions .primary-action,.prompts-heading-actions .secondary-action,.prompts-toolbar .secondary-action { min-height:36px; padding-inline:11px; font-size:11px; }.prompts-heading-actions .primary-action,.prompt-entry-actions .primary-action { background:var(--prompt-snow-accent); border-color:var(--prompt-snow-accent); }.prompts-toolbar { margin:18px 0; }.prompts-search { display:flex; align-items:center; gap:8px; flex:1; max-width:600px; min-height:36px; padding:0 10px; border:1px solid var(--prompt-snow-border); background:var(--prompt-snow-surface-muted); }.prompts-search input { flex:1; border:0; outline:0; background:transparent; color:inherit; font-size:13px; }.prompts-toolbar select { min-height:36px; padding:8px 10px; border:1px solid var(--prompt-snow-border); background:var(--prompt-snow-surface-muted); color:var(--prompt-snow-text); font-size:13px; }.prompts-list { display:grid; gap:10px; }.prompt-entry { padding:16px; border:1px solid var(--prompt-snow-border); background:var(--prompt-snow-surface); box-shadow:var(--prompt-snow-shadow); }.prompt-entry-heading { display:flex; justify-content:space-between; gap:12px; }.prompt-entry-heading h2 { margin:5px 0 0; font-size:16px; }.prompt-entry-heading time { color:var(--prompt-snow-text-muted); font-size:11px; }.prompt-entry-category { color:var(--prompt-snow-accent); font-size:11px; }.prompt-entry p { white-space:pre-wrap; margin:10px 0; color:var(--prompt-snow-text-muted); font-size:13px; line-height:1.5; max-height:90px; overflow:auto; }.prompt-entry-actions { justify-content:flex-end; flex-wrap:wrap; }.prompt-entry-actions .primary-action,.prompt-entry-actions .secondary-action { min-height:34px; padding-inline:11px; font-size:11px; }.prompt-entry-actions .icon-action { width:34px; height:34px; }.prompts-empty { min-height:260px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:8px; color:var(--prompt-snow-text-muted); font-size:13px; }.prompts-empty strong { color:var(--prompt-snow-text); font-size:15px; }
